...
fn main() {
...

    {
        ...
        // if it wasn't, s1 and s2 would both try to free the same memory
        // on the heap when they go out of scope at the same time (double free error)
    ...
    // which means no one owns the original "hello" value,
    ...
    // that means it's copying a value and pushing it again is cheap, so
    // rust doesn't bother invalidating x after it gets copied into y
    let x = 5;
    let y = x;

    // under the hood, simple types that don't require allocations implement Copy
    // anything that can change at runtime and is allocated on the heap
    // has to implement Drop and is banned from implemented Copy
